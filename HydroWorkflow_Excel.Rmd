---
title: "Medsal Project: Analysis of hydrochemical data for the study site Samos"
author: Dimistris Bassukas 
date: 4/2021
geometry: margin=2cm
...
```{r Provide the instructions to run the script, message=FALSE, warning=FALSE, include=FALSE}
...
library(readxl)
water_analysis <-
    read_excel("Hydroworkflow_Input/Application_Samos_water_analyses_with_WK024_25.xlsx")
...
Systematic errors in the sampling procedure and the overall number of suitable or not samples, were evaluated by plotting the distribution of the CBE error.
...
The filtered dataset is retransformed from $mg/L$ to $meq/L$
...
Scope of the cluster analysis is the partition of water samples into clusters (groups) based on similarity, without prior knowledge of groupings between them. Preparation of the dataset to be clustered include:
...
```{r create map, echo=FALSE, fig.height=8, fig.width=10, message=FALSE, warning=FALSE, fig.cap=" Map of the study area with the respective clusters as calculated in the previous section"}
library(ggmap)
library(ggrepel)
...
o18_to_cl = ggplot(data = water_analysis) +
  geom_point(aes(
    x = log(Cl),y =d18O,stroke = 0.1,color = as.factor(Cluster)), alpha = 0.6) +
...
de_to_cl = ggplot(data = water_analysis) +
  geom_point(aes(
    x = log(Cl),y = D_Excess,stroke = 0.1,color = as.factor(Cluster)), alpha = 0.6) +
  ylab(expression(paste("DE(‰)"))) +
  xlab(TeX(r'($log(Cl ^{-})\;meq/L$)'))+ 
  theme_light() +
  theme(legend.position = "right", panel.grid.minor = element_blank()) +
...
```{r Arrange isotope plots into a grid, eval=FALSE, include=FALSE}
library(ggpubr)

composite_isotope_plot_grid = ggarrange(
  deuterium_to_18O,de_to_O18,o18_to_cl ,de_to_cl,
  ncol = 2, 
  nrow = 2, labels = c("A", "B", "C", "D"),
  common.legend = TRUE, legend = "bottom", heights = 10, font.label = list(size = 10)) 

ggsave(file="Composite_isotopic_ratios.svg",  composite_isotope_plot_grid)

composite_plot_grid
```

```{r Speciation calculation, eval=FALSE, include=FALSE}
library(tidyverse)
library(tidyphreeqc)

# Run simple speciation calculations for each sample and save  to .txt file in the working directory

speciation_log <- file("PHREEQC_RUN.txt")

for(i in 0:length(water_analysis$Sample)) {
  print(water_analysis$Sample[i])
  phr_run(
    phr_solution_list(
      pH = 7,
      temp = 25,
      units = "mg/L",
      Na = water_analysis$Na[i],
      K = water_analysis$K[i],
      Mg = water_analysis$Mg[i],
      S = water_analysis$SO4[i],
      C = water_analysis$HCO3[i],
      Cl = water_analysis$Cl[i]
    )
  ) %>%
    phr_print_output()
  
  sink(speciation_log, append = TRUE, type = "output") 
...
A total of 29 hydrochemical analyses were examined from the study area Samos, Greece. Only one site, WK021, presented a \|Charge Balance Error\| above 5% ( -5.5%) and was thus excluded from further analysis.

## Cluster analysis

A Hierarchical cluster analysis was used to group different water samples based on the ions: Cl^-^, HCO~3~^-^, SO~4~^2-^,Ca^2+^, Mg^2+^, Na^+^, K^+^. Prior to clustering, the CBE filtered data was $log_{10}$ transformed and afterwards z-scores were calculated and used as the normalized input for the analysis. The **Manhattan** or **City Block Distance (CDB)** was used as a measure of similarity and **Ward's** method for the agglomeration.

```{r Summaries by cluster, echo=FALSE}
library(ggplot2)

ggplot(data = water_analysis) + 
  geom_bar(mapping = aes(fill = as.factor(type_water), x = as.factor(Cluster)), position = "dodge")+theme_minimal() + scale_fill_brewer(palette = "Set3")
...
```{r summary }
tapply(water_analysis$Cl, water_analysis$Cluster, summary)
```
